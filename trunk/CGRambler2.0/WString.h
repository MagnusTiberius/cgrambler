...
#define WSTRING_MAX_LEN 300

class WString
{
	WCHAR mBuf[WSTRING_MAX_LEN];
	CHAR mLBuf[WSTRING_MAX_LEN];

public:
	WString(LPWSTR str=NULL);
	WString(LPCWSTR str);
	WString(LPSTR str);
	WString(LPCSTR str);
	...
	bool operator==(LPWSTR  str)const;
	bool operator!=(LPWSTR  str)const;
	...
	WString operator+(LPWSTR str)const;
	WString operator+(LPSTR str)const;
	...
	WString  getUpper() const;
	WString  getLower() const ;
