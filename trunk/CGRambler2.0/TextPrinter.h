...
class TextPrinter;
typedef TextPrinter* TextPrinterPtr;

class TextPrinter
{
	static TextPrinterPtr mSingleton;
	static ID3DX10FontPtr mFont;
	static ID3DX10SpritePtr mSprite;
	static CDXUTTextHelperPtr mTextHelper;
	static WString mTextBuf[MAX_PATH];
	static UINT mTextNum;
	static ::ID3D10DevicePtr mDevice;

public:
	static TextPrinterPtr getSingleton();
	static void init(::ID3D10DevicePtr device);
	void Release(void);
	void drawText(const D3DXVECTOR4 &color=D3DXVECTOR4(1.0f,1.0f,0.0f,1.0f));
	void printLine(const WString &str);
	void printData(const WString &name,const D3DXVECTOR4 &data);
	void printData(const WString &name,const D3DXVECTOR3 &data);
	void printData(const WString &name,const D3DXVECTOR2 &data);
	void printData(const WString &name,const float &data);
};
