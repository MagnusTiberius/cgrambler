...
struct GridHashTableCell
{
	D3DXVECTOR3 *mPos;
	GridHashTableCell *mNext;
	float mWeight; 
	int mID;
};

#ifdef USE_SMART_POINTER
typedef SmartPtr<GridHashTableCell> GridHashTableCellPtr;
#else
typedef GridHashTableCell* GridHashTableCellPtr;
#endif
...
class GridHashTable
{
	BYTE *mVertices;
	int mCurID;
	GridHashTableCellPtr *mCells;
	GridHashTableCellPtr mCellsPool;
	float mScale;
	static int mDi[2][2];
	static int mHashPrime[3];
	static int mTableSizes[20];
	int mVerticesNum;
	float mTolerance;
	float mToleranceHalf;
	INTPtr mReplacedID;
	int mTableSize;
	UINT mStride;

private:
	void createCell(int id,const D3DXVECTOR3Ptr vertex,int hashIndices[3],float weight);
	GridHashTableCellPtr searchCell(int id,const D3DXVECTOR3Ptr vertex,const int *lowIndex,const int *curIndex);
	int getGridIndex(int hashIndices[3]);
public:
	GridHashTable(int verticesNum,UINT stride);
	UINT getCellsNum();
	void addVertex(int id, D3DXVECTOR3Ptr vertex,float weight);
	void init(float tolerance);
	INT getReplacedID(int id);
	BYTE* getCellsVB();
