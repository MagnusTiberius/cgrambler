...
Parameter::Parameter(const WString &name,const WString &semantic,const map<WString,WString> &annotatioin,const WString& value,ID3D10EffectVariablePtr &effectVariable)
{
	mSemantic=semantic;
	mName=name;
	mAnnotation=annotatioin;
	mValue=value;
	mEffectVariable=effectVariable;
...
const WString& Parameter::getName()
{
	return mName;
}

const WString& Parameter::getAnnotation(const WString &name)
{
	return mAnnotation[name];
}

ID3D10EffectVariablePtr  Parameter::getEffectVariable()
{
	return mEffectVariable;
}

const WString& Parameter::getSemantic()
{
	return mSemantic;
}


const WString& Parameter::getValue()
{
	return mValue;
}