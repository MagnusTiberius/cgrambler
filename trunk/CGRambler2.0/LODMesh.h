...
#define MESH_LOD_GRIDSIZE_PIXEL_RATIO 5.0f

class LODMesh:
	public  CDXUTSDKMesh
{
	ID3D10BufferPtr mLODIBs[MAX_MESH_LODS];  
	ID3D10BufferPtr mLODVBs[MAX_MESH_LODS];  
	UINT mLODIBLengths[MAX_MESH_LODS];
	MeshLODGeneratorPtr mLODGenerator;
	::ID3D10Device *mDevice;
	WString mFileName;
public:
	LODMesh(ID3D10Device *device,const WString &fileName);
	virtual ~LODMesh(void);
	void getLOD(int level,ID3D10BufferPtr *vb,ID3D10BufferPtr *ib,UINT *numIndices);
	int getAutoLevel(const D3DXVECTOR3& camPos,const D3DXVECTOR3 &meshPos);
	void  Render(struct IDirect3DDevice9 *,struct ID3DXEffect *,char const *,char const *,char const *,char const *){};
};

typedef LODMesh* LODMeshPtr;